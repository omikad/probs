...
void UsageCounter::MarkCheckpoint(string name) {
    ...
    checkpoint_times.push_back(chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now().time_since_epoch()).count());
}
...
void UsageCounter::PrintStats() const {
    ...
    for (const string& name : checkpoint_names) {
        ...
        stringstream tab_time_total; tab_time_total << "time spent " << (double)total_time/1000 << " sec";
