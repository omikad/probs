...
int TransformForPosition(InputFormat input_format,
...
InputPlanes EncodePositionForNN(
  ...
  {
    const ChessBoard& board = history_tree.positions[history_nodes.back()].GetBoard();
    ...
    if (IsHectopliesFormat(input_format)) {
      result[kAuxPlaneBase + 5].Fill(history_tree.positions[history_nodes.back()].GetRule50Ply() / 100.0f);
    } else {
      result[kAuxPlaneBase + 5].Fill(history_tree.positions[history_nodes.back()].GetRule50Ply());
    }
  ...
  bool skip_non_repeats =
      ...
      input_format == 
                          INPUT_112_WITH_CANONICALIZATION_V2_ARMAGEDDON;
  bool flip = false;
  int history_idx = history_nodes.size() - 1;
  for (int i = 0; i < std::min(history_planes, kMoveHistory);
       ++i, --history_idx) {
    const Position& position =
        history_tree.positions[history_nodes[history_idx < 0 ? 0 : history_idx]];
    ...
    // position.
    if (stop_early && history_idx != history_nodes.size() - 1 &&
